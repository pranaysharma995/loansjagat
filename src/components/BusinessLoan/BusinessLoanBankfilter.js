  ...
  components: { 
	...
	methods: {
		applyLoan(){
			...
			axios.post('applied-business-loan',addData)
				.then((response)=>{
		...
		filterSliderBankData(){
			...
			axios.get('business-loan-filter/'+this.id+'/'+this.prce+'/'+this.tenure)
				.then((response)=>{
				this.loader = false
				this.nodata = false
		...
		fetchBanksData(){
			...
			// 	tenure:this.tenure
			// }
			axios.get('business-loan-filter/'+this.id+'/'+this.prce+'/5')
				.then((response)=>{
