  ...
  components: { 
	...
	methods: {
		applyLoan(){
			...
			axios.post('applied-salaried-lap',addData)
				.then((response)=>{
		...
		filterSliderBankData(){
			...
			axios.get('loan-against-property-salaried-banks/'+this.id+'/'+this.value+'/'+this.tenure)
				.then((response)=>{
				this.loader = false
				this.nodata = false
		...
		fetchBanksData(){
			...
			// 	tenure:this.tenure
			// }
			axios.get('loan-against-property-salaried-banks/'+this.id+'/0/15')
				.then((response)=>{
				...
				this.banks = response.data.banks
				this.bank_length = response.data.banks.length
				this.maix_data = response.data.max
