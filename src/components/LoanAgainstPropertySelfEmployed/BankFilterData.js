  ...
  components: { 
	...
	methods: {
		applyLoan(){
			...
			axios.post('applied-self-employed-loan-against-property',addData)
				.then((response)=>{
		...
		filterSliderBankData(){
			...
			// 	tenure:this.tenure
			// }
 			axios.get('employed-property-loan-filter/'+this.id+'/'+this.prce+'/'+this.tenure)
				.then((response)=>{
				this.loader = false
				this.nodata = false
				this.menu = true
		...
		fetchBanksData(){
			...
			// 	tenure:this.tenure
			// }
			axios.get('employed-property-loan-filter/'+this.id+'/0/0')
				.then((response)=>{
